...
let PopoverAnimatorWillShowNotificationName = Notification.Name("XMGPopoverAnimatorWillShow")
let PopoverAnimatorWilldismissNotificationName = Notification.Name("XMGPopoverAnimatorWilldismiss")
...
class PopoverAnimator: NSObject, UIViewControllerTransitioningDelegate, UIViewControllerAnimatedTransitioning {
    ...
    func animationController(forPresented presented: UIViewController, presenting: UIViewController, source: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        ...
        // 发送通知, 通知控制器即将展开
        NotificationCenter.default.post(name: PopoverAnimatorWillShowNotificationName, object: self)
        return self
    }
    ...
    func animationController(forDismissed dismissed: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        ...
        // 发送通知, 通知控制器即将消失
        NotificationCenter.default.post(name: PopoverAnimatorWilldismissNotificationName, object: self)
        return self
    }
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if isPresent {
            ...
            UIView.animate(withDuration: transitionDuration(using: transitionContext), animations: {
                // 注意:由于CGFloat是不准确的, 所以如果y写0.0会没有动画
                // 压扁
